...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    let total = getTotalCartValue(items);
    if(balance<total){
        ...
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box display="flex" flexDirection="column">
              ...
              {addresses.all.map((adr)=>(
                <Box key={adr._id}
                 className= {`address-item ${addresses.selected===adr._id ? "selected" : "not-selected"}`}
                 onClick={()=>(
                  setAddresses({ ...addresses, selected:adr._id})
                 )}>
                  <>{adr.address}</>
                  <Button onClick={()=>deleteAddress(token,adr._id)}>Delete</Button>
                </Box>
              ))}
              </>) :
               (<Typography my="1rem">
